...
**Critical ordering invariant (iter6 — final)**: state.json is written **BEFORE** the webhook reconciliation. This is safe in iter5+ because the skip path was removed — any remote/local drift converges on the next run via the always-PATCH branch. The state-first ordering closes an orphan-webhook failure mode in the CREATE path: if the webhook is created first and then the state write fails (disk full, SIGKILL), the next run cannot find the orphaned hook because it has no stored smee URL, and it creates a second webhook. With state-first, the next run reuses the stored credentials and either finds the hook (PATCH-recovers) or recreates it with the same identity (no orphan). See the `runInstall` comment block in `lib/setup/orchestrator.ts` for the full rationale.
