...
### Skip-idempotency requires the full identifying tuple, not a single field
The iter2 fix tracked "was the secret regenerated this run?" and used that as the skip guard — but that still left two silent failure modes: (a) stored secret with no stored URL + `--smee-url` pointing at an existing hook → the hook at the CLI URL was created with some other secret, not our stored one; (b) stored `{secret, urlA}` + `--smee-url urlB` override → the hook at urlB is unrelated to the stored secret. In both cases `secretWasGenerated === false`, so the installer silently skipped and the runtime failed HMAC validation on every event. The correct invariant is: **you can only skip a network reconciliation when the full identifying tuple (here, `{webhookSecret, smeeUrl}`) was present as a pair in stored state and nothing in it has changed**. Any single-field check is insufficient. The fix replaces `matchingHook && !secretWasGenerated` with `matchingHook && existingState.webhookSecret && existingState.smeeUrl === expectedSmeeUrl`. Whenever you find yourself writing "X was reused, so we can skip," ask: "does X alone identify the remote resource, or does the reconciliation depend on X-paired-with-Y?" *(Spec 3, PR review iter3)*
...
### When you remove a fast path, re-examine its upstream constraints
The "webhook first, state second" ordering was established in iter3 specifically to protect a now-deleted skip path: if state were written first, the next run's skip-idempotency check would be fooled into bypassing a needed PATCH. After iter5 removed the skip path entirely, the "webhook-first" ordering was left in place as a vestige — until iter6 caught the opposite bug in the CREATE path (ghCreateHook success + writeState failure → orphan webhook on next run). The fix was to move writeState back before the webhook step, which is now safe because iter5 removed the reason we had flipped it in the first place. **When you remove one piece of defensive code, audit every other piece of code that was added to work around it — they may now be actively harmful.** The same principle applies to feature flags, deprecations, and any other constraint that cascades through a codebase. *(Spec 3, PR review iter6)*
